...
import com.v2box.mobiletina.AppConfig.LOOPBACK
...
object HttpUtil {
    ...
    fun getUrlContentWithUserAgent(request: UrlContentRequest): String {
        return getUrlContentWithUserAgentAndInfo(request).first
    }

    /** Returns the downloaded body and subscription metadata from the same response. */
    @Throws(IOException::class)
    fun getUrlContentWithUserAgentAndInfo(request: UrlContentRequest): Pair<String, String?> {
        ...
        while (redirects++ < maxRedirects) {
            ...
            val finalUserAgent = if (request.userAgent.isNullOrBlank()) {
                "V2BOX/${BuildConfig.VERSION_NAME}"
            } else {
                request.userAgent
            }
            val requestBuilder = Request.Builder()
