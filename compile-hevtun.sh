...
#    com.v2box.mobiletina.service.TProxyService for the VpnService hev tun mode.
...
"$NDK_HOME/ndk-build" \
    ...
    NDK_OUT="$TMPDIR/obj" \
    "APP_CFLAGS=-O3 -DPKGNAME=com/v2box/mobiletina/service" \
    "APP_LDFLAGS=-Wl,--build-id=none -Wl,--hash-style=gnu" \

# 2) Standalone executable (libhevsockstun.so) — run as a separate root
#    process by com.v2box.mobiletina.core.root for the Root run mode. Same hev source,
